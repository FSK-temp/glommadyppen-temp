...
# ── Asymmetrisk kaldhale (relaksasjon etter dipp) ───────────────────────────
# En kaldpuls ved Fetsund er ikke et punkt: gjeninnhentingen skjer ved
# soloppvarming og innblanding og tar tid. Empirisk (n = 55 episoder, times-
# data 2015–2025) er tidskonstanten τ ≈ 38 t, og den skalerer med dybden på
# dippen: ~32 t for 2,5–4 °C, ~51 t for 4–6 °C, ~75 t for de kraftigste.
# Nedsiderisikoen fra vind glattes derfor bakover i tid med denne τ, slik at
# båndet får en realistisk lang hale i stedet for en enkelt nål.
RECOVERY_TAU_HOURS      = 38.0    # median gjeninnhentings-tidskonstant, Fetsund
RECOVERY_TAU_PER_DEG    = 7.0     # ekstra timer τ per °C dippdybde over 3 °C
RECOVERY_LOOKBACK_HOURS = 96.0    # hvor langt bakover en kaldpuls smøres
...
def build_fetsund_forecast(vorma_df, fetsund_df, discharge_df,
    ...
    for h_step in range(0, hours_ahead + 1, step_h):
        ...
        # ── Asymmetriske bånd ────────────────────────────────────────────────
        # Øvre grense: KUN grunnusikkerheten. Vind kan aldri gjøre vannet
        # varmere (oppvelling henter kaldt dypvann), så vindmultiplikatoren og
        # risk_shift skal ikke påvirke taket. Tidligere ble sigma_eff (med
        # vindmultiplikator) brukt begge veier, noe som ga en fysisk umulig
        # oppadgående spiss samtidig med kaldpulsen.
        sigma_up   = sigma
        # Nedre grense: grunnusikkerhet forsterket av vind, pluss et midt-skifte
        # (risk_shift) som trekker selve nedsidegrensen ned uten å flytte pred.
        sigma_down_here = sigma * sigma_mult

        rows.append({
            'time':            t_fut,
            'predicted':       round(pred, 2),
            'sigma_up':        sigma_up,
            'sigma_down':      sigma_down_here,
    ...
    # ── Temporal persistens: asymmetrisk kaldhale ────────────────────────────
    # En kaldpuls som treffer tidssteg i påvirker også nabotidsstegene: pulsen
    # har utstrekning, og gjeninnhentingen etterpå er treg (τ ≈ 38 t). Vi lar
    # derfor NEDSIDEN på hvert tidssteg være konvolusjonen av alle tidligere
    # (og nære fremtidige) nedsidebidrag, vektet med en asymmetrisk kjerne som
    # faller raskt fremover (pulsens forkant) og sakte bakover (den lange halen).
    # Resultatet er et sammenhengende bånd som henger ned, ikke en enkelt nål.
    t_arr    = out['time'].values.astype('datetime64[h]').astype('int64')
    depth    = (out['risk_shift'].abs() + (out['sigma_down'] - out['sigma_up']).clip(lower=0)).values
    # τ skaleres med lokal dippdybde (dypere puls -> lengre hale)
    eff_low  = np.zeros(len(out))
    for i in range(len(out)):
        dt_h  = t_arr - t_arr[i]                      # >0 = fremtid, <0 = fortid
        tau_i = RECOVERY_TAU_HOURS + RECOVERY_TAU_PER_DEG * max(0.0, depth[i] - 3.0)
        # bakover (dt<0): lang hale med tau_i ; fremover (dt>0): bratt forkant (tau/3)
        w = np.where(
            dt_h <= 0,
            np.exp(dt_h / tau_i),                     # dt_h negativ -> avtar bakover
            np.exp(-dt_h / (tau_i / 3.0)),            # bratt fremkant
        )
        w[np.abs(dt_h) > RECOVERY_LOOKBACK_HOURS] = 0.0
        eff_low = np.maximum(eff_low, depth[i] * w)   # maks-konvolusjon (envelope)

    # Bygg endelige asymmetriske grenser
    pred_v   = out['predicted'].values
    su       = out['sigma_up'].values
    sd       = out['sigma_up'].values + eff_low       # nedside = grunn + utsmurt kaldhale
    lower68  = np.maximum(pred_v - sd,         TEMP_HIST_LOWER)
    upper68  = np.minimum(pred_v + su,         TEMP_HIST_UPPER)
    lower95  = np.maximum(pred_v - 1.96 * sd,  TEMP_HIST_LOWER)
    upper95  = np.minimum(pred_v + 1.96 * su,  TEMP_HIST_UPPER)

    out['lower_68'] = np.round(lower68, 2)
    out['upper_68'] = np.round(upper68, 2)
    out['lower_95'] = np.round(lower95, 2)
    out['upper_95'] = np.round(upper95, 2)
    return out[['time', 'predicted', 'lower_68', 'upper_68', 'lower_95',
                'upper_95', 'wind_E_forecast', 'wind_risk_level']]
