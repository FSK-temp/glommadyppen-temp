...
# Sett FORECAST_MODE = 'level' for å rulle tilbake med én linje.
FORECAST_MODE            = 'increment'
...
def build_fetsund_forecast(vorma_df, fetsund_df, discharge_df,
    ...
    def _vorma_at(t):
        """T_Vorma på tidspunkt t. Utenfor dataserien: eksponentiell relaksasjon
        av anomalien mot 72t-medianen (e-foldingstid VORMA_RELAX_HOURS)."""
    ...
    # Referansepunkt: vannet som ankommer Fetsund akkurat nå
    ...
    for h_step in range(0, int(hours_ahead) + 1, int(step_h)):
        ...
        if use_mode == 'increment':
            ...
            # Behold den myke overgangen fra nå-tilstand for nivåformen
        ...
        sigma_extrap = MODEL_SIGMA_ASYMPTOTE * float(
            np.sqrt(1.0 - np.exp(-extrap / SIGMA_EXTRAP_TAU)))
        sigma = float(np.hypot(sigma_data, sigma_extrap))
